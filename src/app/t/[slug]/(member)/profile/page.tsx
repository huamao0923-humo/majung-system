...
export default async function TenantProfilePage({
  ...
  const reservationCount = await prisma.reservation.count({
    where: { userId: user.id, tenantId: tenant.id, status: "completed" },
  });
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      >
        <Link href={`/t/${slug}`} className="p-1 -ml-1" style={{ color: "rgba(212,175,55,0.8)" }}>
      ...
      <div className="px-4 py-6 space-y-4 pb-8" style={{ background: "#FDF6E3", minHeight: "calc(100vh - 56px)" }}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <p className="text-sm mt-1" style={{ color: "rgba(212,175,55,0.6)" }}>已完成預約</p>
          </div>
          <div
            className="rounded-2xl p-5 text-center shadow-sm"
            style={{ background: "white", border: "1px solid rgba(212,175,55,0.2)" }}
          >
            ...
            <p className="text-sm mt-1" style={{ color: "rgba(139,0,0,0.5)" }}>未到場次數</p>
          </div>
        </div>
