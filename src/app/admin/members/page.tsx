...
function MemberModal({
  ...
  const [noShowCount, setNoShowCount] = useState(String(member.noShowCount));
  const [isBlacklisted, setIsBlacklisted] = useState(member.isBlacklisted);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      await onSave({ displayName, phone, noShowCount: parseInt(noShowCount) || 0, isBlacklisted });
      onClose();
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl" style={{ background: "white" }}>
        <div className="px-6 py-4" style={{ background: "linear-gradient(135deg, #1A0500, #0D0200)", borderBottom: "1px solid #D4AF3725" }}>
          <h2 className="text-base font-semibold" style={{ color: "#D4AF37" }}>編輯會員</h2>
        </div>
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-medium mb-1" style={{ color: "#6B4C2A" }}>姓名 <span style={{ color: "#8B0000" }}>*</span></label>
          ...
          <div>
            ...
            <input type="number" min="0" value={noShowCount} onChange={(e) => setNoShowCount(e.target.value)}
              className="w-full px-3 py-2 rounded-xl text-sm outline-none"
              style={{ background: "#FDF6E3", border: "1px solid rgba(212,175,55,0.3)", color: "#1A0500" }} />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm" style={{ color: "#1A0500" }}>{isBlacklisted ? "黑名單（禁止預約）" : "狀態正常"}</span>
          </label>
          {error && <p className="text-xs px-3 py-2 rounded-lg" style={{ background: "#FEE2E2", color: "#991B1B" }}>{error}</p>}
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-medium"
              style={{ background: "#F3F4F6", color: "#374151" }}>取消</button>
            <button type="submit" disabled={saving} className="flex-1 py-2.5 rounded-xl text-sm font-medium disabled:opacity-60"
...
function DeleteConfirmModal({ memberName, onClose, onConfirm }: { memberName: string; onClose: () => void; onConfirm: () => Promise<void> }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="w-full max-w-xs rounded-2xl overflow-hidden shadow-2xl" style={{ background: "white" }}>
        <div className="px-6 py-5 text-center space-y-3">
          <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto" style={{ background: "#FEE2E2" }}>
            <AlertTriangle className="w-6 h-6" style={{ color: "#DC2626" }} />
          </div>
          <p className="text-sm font-medium" style={{ color: "#1A0500" }}>確定要刪除「{memberName}」？</p>
          <p className="text-xs" style={{ color: "#9CA3AF" }}>此操作無法復原，相關預約記錄也會一併刪除。</p>
        </div>
        <div className="flex border-t" style={{ borderColor: "#F3F4F6" }}>
          <button onClick={onClose} className="flex-1 py-3 text-sm font-medium" style={{ color: "#6B7280" }}>取消</button>
          <div className="w-px" style={{ background: "#F3F4F6" }} />
          <button onClick={handleConfirm} disabled={deleting} className="flex-1 py-3 text-sm font-semibold disabled:opacity-60" style={{ color: "#DC2626" }}>
            {deleting ? "刪除中…" : "確定刪除"}
          </button>
        </div>
      </div>
...
export default function AdminMembersPage() {
  ...
  const fetchMembers = useCallback(async (currentPage: number, currentSearch: string) => {
    setLoading(true);
    try {
      const sp = new URLSearchParams({ page: String(currentPage), ...(currentSearch ? { q: currentSearch } : {}) });
      const res = await fetch(`/api/admin/members?${sp}`);
      if (res.ok) {
  ...
  async function handleDelete() {
    ...
    const res = await fetch(`/api/users/${deleteMember.id}`, { method: "DELETE" });
  ...
  async function handleCleanDemo() {
    ...
    try {
      const res = await fetch("/api/admin/members/clean-demo", { method: "DELETE" });
      if (res.ok) {
    ...
    <div className="p-6 space-y-5 max-w-4xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={handleCleanDemo} disabled={cleaningDemo}
      ...
      {/* 搜尋列 */}
      <form onSubmit={handleSearch} className="rounded-2xl p-4 shadow-sm"
        ...
        <div className="flex gap-2">
          <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)}
            ...
            style={{ background: "#FDF6E3", border: "1px solid rgba(212,175,55,0.3)", color: "#1A0500" }} />
          <button type="submit" className="px-4 py-2 rounded-xl text-sm font-medium transition-opacity hover:opacity-85"
            style={{ background: "linear-gradient(135deg, #8B0000, #6B0000)", color: "#D4AF37" }}>搜尋</button>
          {search && (
            <button type="button" onClick={() => { setSearchInput(""); setSearch(""); setPage(1); }}
              className="px-4 py-2 rounded-xl text-sm font-medium"
              style={{ background: "#F3F4F6", color: "#6B7280" }}>清除</button>
          )}
        </div>
      </form>
      ...
      {/* 列表 */}
      {loading ? (
        <div className="rounded-2xl text-center py-14 text-sm"
          style={{ background: "white", border: "1px solid #D4AF3720", color: "rgba(139,0,0,0.3)" }}>載入中…</div>
      ) : members.length === 0 ? (
        <div className="rounded-2xl text-center py-14 text-sm"
          style={{ background: "white", border: "1px solid #D4AF3720", color: "rgba(139,0,0,0.3)" }}>找不到符合的會員</div>
      ) : (
        <div className="rounded-2xl overflow-hidden shadow-sm" style={{ border: "1px solid #D4AF3720" }}>
          <div className="px-5 py-3 grid items-center text-xs font-semibold uppercase"
            style={{
          ...
          <div className="bg-white divide-y" style={{ borderColor: "#D4AF3715" }}>
            {members.map((m) => (
              <div key={m.id} className="px-5 py-3 grid items-center hover:bg-amber-50/30 transition-colors"
                ...
                <div className="text-center">
                  {m.isBlacklisted ? (
                    <span className="inline-block text-xs px-2 py-0.5 rounded-full font-medium"
                      style={{ background: "#FEE2E2", color: "#991B1B", border: "1px solid #FCA5A5" }}>黑名單</span>
                  ) : (
                    <span className="inline-block text-xs px-2 py-0.5 rounded-full font-medium"
                      style={{ background: "#F0FDF4", color: "#166534", border: "1px solid #BBF7D0" }}>正常</span>
                  )}
                </div>
                <div className="flex items-center justify-center gap-1.5">
                  <button onClick={() => setEditMember(m)}
                    className="p-1.5 rounded-lg transition-colors hover:bg-amber-50" title="編輯"
                    style={{ color: "#8B6914" }}>
                    <Pencil className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => setDeleteMember(m)}
                    className="p-1.5 rounded-lg transition-colors hover:bg-red-50" title="刪除"
                    style={{ color: "#DC2626" }}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
              ...
              style={{ background: "linear-gradient(135deg, #8B0000, #6B0000)", color: "#D4AF37" }}>上一頁</button>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
              className="px-3 py-1.5 rounded-lg text-xs font-medium disabled:opacity-40"
              style={{ background: "linear-gradient(135deg, #8B0000, #6B0000)", color: "#D4AF37" }}>下一頁</button>
          </div>
        </div>
      )}
      ...
      {deleteMember && (
        <DeleteConfirmModal memberName={deleteMember.displayName} onClose={() => setDeleteMember(null)} onConfirm={handleDelete} />
      )}
    </div>
  );
}
