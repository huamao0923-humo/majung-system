...
const navItems = [
  ...
  { href: "/reserve", icon: CalendarPlus, label: "預約" },
  { href: "/my-reservations", icon: CalendarDays, label: "我的預約" },
  { href: "/profile", icon: User, label: "會員" },
];
...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50"
      style={{ background: "linear-gradient(180deg, #FFFBF0, #FDF6E3)", borderTop: "1px solid #D4AF3740", boxShadow: "0 -4px 20px #8B000015" }}>
      <div className="grid grid-cols-4 max-w-lg mx-auto">
        {navItems.map(({ href, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              {active && (
                <div className="absolute h-[2px] w-10 -mt-3 rounded-full"
              ...
              <span className="font-medium" style={{ fontSize: "11px", color: active ? "#8B0000" : "#9CA3AF" }}>
                {label}
              </span>
            </Link>
          );
